...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoader(true)
      try {
        const benifitData = await axios.get(`${Base_url}benefits/`)
        setBenifits(benifitData.data.data.slice(0,3))

        const coursesData = await axios.get(`${Base_url}courses/`)
        console.log(coursesData)
        setCourses(coursesData.data.data.slice(0,6))
      } catch (error: any) {
        console.log(error)
      }
      finally {
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {courses.map((course) => (
              <OurCoursesCard
                key={course.id}
                imageurl={typeof course.images[0] === 'object' && 'image' in course.images[0] ? (course.images[0] as { image: string }).image : ''} // Type check and assertion
